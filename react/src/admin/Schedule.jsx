...
import { CreateScheduleModal, UpdateScheduleModal, DeleteScheduleModal } from './modal_admin/ModalSchedule';

const Schedule = () => {

    const TABLE_HEAD = ["No", "Id", "Name", "StartDate", "EndDate", "Action"];
    ...
    return (
        <div className='pt-6 px-6 md:pt-8 md:px-8'>
                        ...
                        <tbody>
                            {TABLE_ROWS.map((row, index) => (
                                <tr key={index} >
                                    ...
                                    <td className={tdTable}>{row.start_date}</td>
                                    <td className={tdTable}>{row.end_date}</td>
            ...
            <CreateScheduleModal visible={showModalCreate} onClose={() => setShowModalCreate(false)} />
            <UpdateScheduleModal visible={showModalUpdate} onClose={() => setShowModalUpdate(false)} />
            <DeleteScheduleModal visible={showModalDelete} onClose={() => setShowModalDelete(false)} />
        </div>
  )
}
